...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const squareModuleX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const squareModuleY = Math.sqrt(x2 ** 2 + y2 ** 2);
  const angle = dotProduct / (squareModuleX * squareModuleY);
...
function roundToPowerOfTen(num, pow) {
  const round = 10 ** pow;
  return Math.round(num / 10 ** pow).toFixed(pow) * round;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.round((x1 + x2 + x3) * 100000) / 100000;
}
